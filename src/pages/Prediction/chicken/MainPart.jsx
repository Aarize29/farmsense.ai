...
import { model_predict } from '../../../data_portal';
...
const ChickenMainPart = () => {
  ...
  const addDisease = async(e) => {
    try{
      const userdata= await addDoc(collection(db, "chickendisease"), {
  ...
  const handlPrediction= async(e)=>{
    ...
    const prediction = await model_predict(base64, "poultry_diseases")
